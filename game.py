...
class GameWindow(wx.Window):
    ...
    def StartGame(self, word):
        ...
        self.tries = 0
        self.misses = 0
        self.hint = u"".join(random.sample(word, int(len(word)*0.5)))
        self.Draw()
    ...
    def Draw(self, dc = None):
        ...
        dc.DrawText("hint: " + self.hint, x1, 70)
        dc.DrawText("guessed:", x1, 90)
...
class GameFrame(wx.Frame):
    def __init__(self, parent, wr):
        self.wr = wr
        wx.Frame.__init__(self, parent, -1, u'Hangman', wx.DefaultPosition, (500,600))

        self.wnd = GameWindow(self, -1)
        ...
        quit = wx.MenuItem(menu, 1005, '&Quit\tCtrl+Q', 'Quit the Application')
    ...
    def OnWindowClose(self, event):
        dlg = wx.MessageDialog(self, 'Do you want close this window?', '', wx.YES_NO | wx.YES_DEFAULT | wx.CANCEL | wx.ICON_QUESTION)
        val = dlg.ShowModal()
